...
def getHistoryTest(level):
    ...
    if level == "easy":
        ...
        print("when did WW2 srart and end? a - 1 September 1939 to 2 September 1945 or b - 31 April 1956 to 19 december 1969? ")
        if isAnswerCorrect(getAnswer(level), "a"):
            score += 1
        print("Your score it", score)

        print("when was the treaty of Versilles signed? a - March 20 1920 or b - June 28, 1919? ")
        if isAnswerCorrect(getAnswer(level), "b"):
            score +=1
        print("Your score it", score)

        print("Which countries took part in WW1?  a - Britain, France, Russia, taly, Germany, Austria-Hungary, Ottoman Empire and Bulgaria or b - China, Britain, France, Brazil, lithuania, Germany Sweden and Finland? ")
    else:
        print("No test found")
